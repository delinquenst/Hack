...
About.TextSize = 14
About.Text = "Plugins are .ty files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the TY Discord (discord.io/infiniteyield)"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with TY,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
		...
		if pcall(function() readfile("TY_FE." .. fileExtension) end) then
			if readfile("TY_FE." .. fileExtension) ~= nil then
				local success, response = pcall(function()
					local json = game:GetService("HttpService"):JSONDecode(readfile("TY_FE." .. fileExtension))
					...
					if json.keepTY ~= nil then KeepInfYield = json.keepTY else KeepInfYield = true end
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("TY_FE." .. fileExtension, game:GetService("HttpService"):JSONEncode(update))
	end
end
...
local function clicktpFunc()
	pcall(function()
		...
		getRoot(Players.LocalPlayer.Character).CFrame = TYMouse.Hit + Vector3.new(0,7,0)
	end)
end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if State == Enum.TeleportState.Started then
		if KeepInfYield and queueteleport then
			queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeTY/infiniteyield/master/source'))()")
		end
	end
end)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(TYMouse.X, TYMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('tptool', {'teleporttool'}, function(args, speaker)
	...
	TpTool.Activated:Connect(function()
		...
		HRP.CFrame = CFrame.new(TYMouse.Hit.X, TYMouse.Hit.Y + 3, TYMouse.Hit.Z, select(4, HRP.CFrame:components()))
	end)
end)
	...
	if sethidden then		
		simRadLoop = game:GetService('RunService').Stepped:Connect(function()
			if setsimulation then
				notify('Dont even think about it','Using simulationradius will make roblox take a screenshot of your PC, and will remove network ownership.)')
			else	
                notify('Dont even think about it','Using simulationradius will make roblox take a screenshot of your PC, and will remove network ownership.)')
			end
		end)
